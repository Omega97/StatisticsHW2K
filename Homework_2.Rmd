...
Now we can compute both $t_obs$ and the $P-value$
```{r}
m1 = 22.30
m2 = 25.91
n1 = 43
...
$$
(\bar{x_2} - \bar{x_1}) \pm t_{n_1+n_2-2,\alpha/2} \cdot SE = (\bar{x_2} - \bar{x_1}) \pm t_{n_1+n_2-2,\alpha/2} \cdot s\sqrt{\frac{1}{n_1}+\frac{1}{n_2}}
$$
...
**Solution**
In building a confidence interval based on the Wald test for a binomial distribution, the parameter of interest is estimated by approximating the error distribution with a normal distribution. Therefore, by definition of the estimate of the parameter of interest $p_0$, the resulting interval will be symmetric around $p_0$ itself (mean of the distribution):
$$
p_0 \pm z_{\alpha/2} \sqrt{\frac{p_0(1-p_0)}{n}}
$$ 
As stated in literature, the performance of Wald test-based confidence interval can be poor with certain choices of binomial parameters, and in general when the sample size is not big enough or when the sought after proportion approaches its boundaries (tends to 0 or 1). In such scenarios, the symmetry of the unbounded error distribution can generate incompatible intervals, i.e. exceeding the boundaries, as obtained in the following snippet.
